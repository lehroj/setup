...
local DOTFILES_GIT=$HOME/Setup/dotfiles/git

for FILE in $(ls -A1 $DOTFILES_GIT); do
  SRC=$DOTFILES_GIT/$FILE
  DST=$HOME/$FILE
  ...
  if [ -L $DST ] || [ -f $DST ]; then
    ...
    if [ $REPLY == "o" || $REPLY == "O" ]; then
      ...
      echo -e "\e[1;32m✓\e[0m \e[1mSymlink|file overwritten for $FILE\e[0m"
    else
